...
namespace clas12 {
  ...
  void   particle_detector::init(const char *bankName, hipo::reader &r, std::string items){
    //if items exists use it to select only banks you want to use
    readItems(items);

    initBranches(bankName,r); //init bank
    //base detector need detector and pindex items
    _detector_id_order = getEntryOrder("detector");
